...
start_stability = 1  # 初始稳定性
first_interval = 1  # 初始间隔
forget_stability = 1  # 遗忘稳定性
forget_interval = 1  # 遗忘间隔
...
for day in range(0, learn_days):
    ...
    if day + 1 >= period_len:
        workload_per_day_average_per_period[day] = np.true_divide(workload_per_day[day - period_len + 1:day + 1].sum(),
                                                                  period_len)
        new_card_per_day_average_per_period[day] = np.true_divide(new_card_per_day[day - period_len + 1:day + 1].sum(),
                                                                  period_len)
    else:
    ...
    for card in range(0, card_per_day[day]['new'] + card_per_day[day]['forget']):
        ...
        while next_due < learn_days and next_interval < interval_limit:
            ...
            next_due += int(round(next_interval))
...
plt.plot(learn_rate_per_day_average_per_period)
plt.xlabel("时间/天")
